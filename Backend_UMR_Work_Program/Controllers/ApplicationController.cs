                ...
                if (selectedApps != null)
                {
                    foreach (var b in selectedApps)
                    {
                        ...
                        if(await _appProcessFlowService.SendBackApplicationToCompany(Company, application, currentStaff, TypeOfPaymentId, AmountNGN, AmountUSD, comment, selectedTables))
                        {
                            ...
                            _helpersController.SaveHistory(application.Id, currentStaff.StaffID, GeneralModel.SentBackToCompany, comment);
                           
                            string subject = $"Sending back WORK PROGRAM application with ref: {application.ReferenceNo} ({concession.Concession_Held} - {application.YearOfWKP}).";
