    ...
    public class WorkProgrammeController : ControllerBase
    {
        ...
        public async Task<object> GET_FORM_TWO_RESERVES(string omlName, string fieldName, string year)
        {
            try
            {
                ...
                if ((concessionField?.Consession_Type == "OML" || concessionField?.Consession_Type == "PML") && concessionField.Field_Name != null)
                {
                    ...
                    var FiveYearProjection = await (from c in _context.RESERVES_UPDATES_OIL_CONDENSATE_Fiveyear_Projections where c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.OML_Name.ToUpper() == omlName.ToUpper() && c.Year_of_WP == year select c).ToListAsync();
                    ...
                    var ReservesReplacementRatio = await (from c in _context.RESERVES_REPLACEMENT_RATIOs where c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.OML_Name.ToUpper() == omlName.ToUpper() && c.Year_of_WP == year select c).ToListAsync();
                ...
                else
                {
                    ...
                    var FiveYearProjection = await (from c in _context.RESERVES_UPDATES_OIL_CONDENSATE_Fiveyear_Projections where c.OML_Name == omlName.ToUpper() && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    ...
                    var ReservesReplacementRatio = await (from c in _context.RESERVES_REPLACEMENT_RATIOs where c.OML_Name == omlName.ToUpper() && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
        ...
        public async Task<object> GET_FORM_TWO_OIL_PRODUCTION(string omlName, string fieldName, string year)
        {
            try
            {
                ...
                if ((concessionField?.Consession_Type == "OML" || concessionField?.Consession_Type == "PML") && concessionField.Field_Name != null)
                {
                    var OilCondensateProduction = await (from c in _context.OIL_CONDENSATE_PRODUCTION_ACTIVITIEs where c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.OML_Name.ToUpper() == omlName.ToUpper() && c.Year_of_WP == year select c).ToListAsync();
                    //var Unitization = await (from c in _context.OIL_CONDENSATE_PRODUCTION_ACTIVITIES_UNITIZATIONs where c.OML_Name == omlName && c.COMPANY_ID == WKPCompanyId && c.Field_ID == concessionField.Field_ID && c.Year_of_WP == year select c).FirstOrDefaultAsync();
                ...
                {
                    var OilCondensateProduction = await (from c in _context.OIL_CONDENSATE_PRODUCTION_ACTIVITIEs where c.OML_Name == omlName && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    //var Unitization = await (from c in _context.OIL_CONDENSATE_PRODUCTION_ACTIVITIES_UNITIZATIONs where c.OML_Name == omlName && c.COMPANY_ID == WKPCompanyId && c.Field_ID == concessionField.Field_ID && c.Year_of_WP == year select c).FirstOrDefaultAsync();
        ...
        public async Task<object> GET_FORM_FIVE_HSE(string omlName, string fieldName, string year, string type_of_facility, string number_of_facilities)
        {
            try
            {
                ...
                {
                    var HSEQuestion = await (from c in _context.HSE_QUESTIONs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEFatality = await (from c in _context.HSE_FATALITIEs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEDesignSafety = await (from c in _context.HSE_DESIGNS_SAFETies where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEInspectionMaintenance = await (from c in _context.HSE_INSPECTION_AND_MAINTENANCE_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEInspectionMaintenanceFacility = await (from c in _context.HSE_INSPECTION_AND_MAINTENANCE_FACILITY_TYPE_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSETechnicalSafety = await (from c in _context.HSE_TECHNICAL_SAFETY_CONTROL_STUDIES_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSESafetyStudies = await (from c in _context.HSE_SAFETY_STUDIES_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSERemediationFund = await (from c in _context.HSE_REMEDIATION_FUNDs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.Company_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEAssetRegister = await (from c in _context.HSE_ASSET_REGISTER_TEMPLATE_PRESCRIPTIVE_EQUIPMENT_INSPECTION_STRATEGY_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEOilSpill = await (from c in _context.HSE_OIL_SPILL_REPORTING_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEAssetRegisterRBI = await (from c in _context.HSE_ASSET_REGISTER_TEMPLATE_RBI_EQUIPMENT_INSPECTION_STRATEGY_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEAccidentIncidence = await (from c in _context.HSE_ACCIDENT_INCIDENCE_REPORTING_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEAccidentIncidenceType = await (from c in _context.HSE_ACCIDENT_INCIDENCE_REPORTING_TYPE_OF_ACCIDENT_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    ...
                    var HSECommunityDisturbance = await (from c in _context.HSE_COMMUNITY_DISTURBANCES_AND_OIL_SPILL_COST_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEEnvironmentalStudies = await (from c in _context.HSE_ENVIRONMENTAL_STUDIES_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEWasteManagement = await (from c in _context.HSE_WASTE_MANAGEMENT_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEWasteManagementType = await (from c in _context.HSE_WASTE_MANAGEMENT_TYPE_OF_FACILITY_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEProducedWaterMgt = await (from c in _context.HSE_PRODUCED_WATER_MANAGEMENT_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEEnvironmentalCompliance = await (from c in _context.HSE_ENVIRONMENTAL_COMPLIANCE_MONITORING_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();


                    var HSEEnvironmentalFiveYears = await (from c in _context.HSE_ENVIRONMENTAL_STUDIES_FIVE_YEAR_STRATEGIC_PLAN_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSESustainableDev = await (from c in _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_PLANNED_AND_ACTUALs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEEnvironmentalStudiesUpdated = await (from c in _context.HSE_ENVIRONMENTAL_STUDIES_NEW_UPDATEDs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEOSPRegistrations = await (from c in _context.HSE_OSP_REGISTRATIONS_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEProducedWaterMgtUpdated = await (from c in _context.HSE_PRODUCED_WATER_MANAGEMENT_NEW_UPDATEDs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEEnvironmentalComplianceChemical = await (from c in _context.HSE_ENVIRONMENTAL_COMPLIANCE_MONITORING_CHEMICAL_USAGE_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSECausesOfSpill = await (from c in _context.HSE_CAUSES_OF_SPILLs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    ...
                    var HSESustainableDevProgramCsr = await (from c in _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_CSR_NEWs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSESustainableDevScheme = await (from c in _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_SCHOLASHIP_SCHEMEs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEManagementPosition = await (from c in _context.HSE_MANAGEMENT_POSITIONs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEQualityControl = await (from c in _context.HSE_QUALITY_CONTROLs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEClimateChange = await (from c in _context.HSE_CLIMATE_CHANGE_AND_AIR_QUALITies where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSESafetyCulture = await (from c in _context.HSE_SAFETY_CULTURE_TRAININGs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEOccupationalHealth = await (from c in _context.HSE_OCCUPATIONAL_HEALTH_MANAGEMENTs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEWasteManagementSystems = await (from c in _context.HSE_WASTE_MANAGEMENT_SYSTEMs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEWastManagementDZs = await (from c in _context.HSE_WASTE_MANAGEMENT_DZs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    var HSEEnvironmentalManagementSystems = await (from c in _context.HSE_ENVIRONMENTAL_MANAGEMENT_SYSTEMs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                    //Added by Musa

                    var HSEOperationSafetyCases = await (from c in _context.HSE_OPERATIONS_SAFETY_CASEs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();


                    var HSEPointSourceRegistrations = await (from c in _context.HSE_POINT_SOURCE_REGISTRATIONs where c.OML_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.Company_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEEnvironmentalMgtPlans = await (from c in _context.HSE_ENVIRONMENTAL_MANAGEMENT_PLANs where c.OmL_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEEnfluenceConliences = await (from c in _context.HSE_EFFLUENT_MONITORING_COMPLIANCEs where c.OmL_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();


                    var HSEGHGPlans = await (from c in _context.HSE_GHG_MANAGEMENT_PLANs where c.OmL_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.CompanY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();

                    var HSEHostCommunities = await (from c in _context.HSE_HOST_COMMUNITIES_DEVELOPMENTs where c.OmL_ID == concessionField.Concession_ID.ToString() && c.Field_ID == concessionField.Field_ID && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToListAsync();
                ...
                else
                {
                    ...
                    var HSEOperationSafetyCases = (from c in _context.HSE_OPERATIONS_SAFETY_CASEs where c.OML_Name == omlName && c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year select c).ToList();
        ...
        public async Task<object> POST_HSE_EFFLUENT_MONITORING_COMPLIANCE([FromForm] HSE_EFFLUENT_MONITORING_COMPLIANCE Effluenct_Monitoring_Complience_Mode, string omlName, string fieldName, string year, string actionToDo = null, int id = 0)
        {
            int save = 0;
            int Id = id == 0 ? Effluenct_Monitoring_Complience_Mode.Id : id;
            ...
            try
            {
                ...
                if (Id > 0)
                {
                    getData = await (from c in _context.HSE_EFFLUENT_MONITORING_COMPLIANCEs where c.Id == Id select c).FirstOrDefaultAsync();
                    if (getData != null)
                    {
                        if (action == GeneralModel.Delete.ToLower())
                        {
                        ...
                        else
                        {
                            ...
                            getData.AreThereEvidentOfSampling = Effluenct_Monitoring_Complience_Mode.AreThereEvidentOfSampling;
                            getData.OmL_ID = Effluenct_Monitoring_Complience_Mode.OmL_ID;
                            getData.ReasonForNoEvidenceSampling = Effluenct_Monitoring_Complience_Mode.ReasonForNoEvidenceSampling;
                            ...
                            if (files.Count >= 1)
                            {
                                ...
                                if (file1 != null)
                                {
                                    ...
                                    getData.EvidenceOfSamplingPath = await blobService.UploadFileBlobAsync("documents", file1.OpenReadStream(), file1.ContentType, $"EffluenceMonitoringDocuments/{blobname1}", docName.ToUpper(), (int)WKPCompanyNumber, int.Parse(year));
                                    if (getData.EvidenceOfSamplingPath == null)
                                        return BadRequest(new { message = "Failure : An error occured while trying to upload " + docName + " document." });
                                    else
                                        getData.EvidenceOfSamplingFilename = docName;
                                }
                                else
                                {
                                    getData.EvidenceOfSamplingPath = null;
                                    getData.EvidenceOfSamplingFilename = null;
                                }
                            }
                            else
                            {
                            ...
                            {
                                getData.ReasonForNoEvidenceSampling = null;
                            }
                            else
                            {
                                getData.EvidenceOfSamplingPath = null;
                            ...
                            #endregion
                            _context.HSE_EFFLUENT_MONITORING_COMPLIANCEs.Update(getData);
                            save += await _context.SaveChangesAsync();
                ...
                else if (Effluenct_Monitoring_Complience_Mode != null)
                {
        ...
        public async Task<object> POST_HSE_GHG_MANAGEMENT_PLAN([FromForm] HSE_GHG_MANAGEMENT_PLAN ghg_Mgt_Plan_Model, string omlName, string fieldName, string year, string actionToDo = null, int id = 0)
        {
            ...
            int save = 0;
            int Id = ghg_Mgt_Plan_Model.Id != null ? ghg_Mgt_Plan_Model.Id : 0;
            string action = (actionToDo == null || actionToDo == "") ? GeneralModel.Insert : actionToDo.Trim().ToLower();
            var concessionField = GET_CONCESSION_FIELD(omlName, fieldName);
            ...
            try
            {
                ...
                #region Saving Operation Safety Case
                if (ghg_Mgt_Plan_Model != null || id > 0)
                {

                    if (id > 0 && action == GeneralModel.Delete)
                    {
                        getOperationSafetyCaseData = await (from c in _context.HSE_GHG_MANAGEMENT_PLANs where c.Id == id select c).FirstOrDefaultAsync();
            ...
            try
            {
                if (Id > 0)
                {
                    ...
                    if (getData != null)
                    {
                        if (action == GeneralModel.Delete.ToLower())
                        {
                            _context.HSE_HOST_COMMUNITIES_DEVELOPMENTs.Remove(getData);
                            save += _context.SaveChanges();
                            string successMsg = Messager.ShowMessage(GeneralModel.Delete);
                            return new WebApiResponse { ResponseCode = AppResponseCodes.Success, Message = successMsg, StatusCode = ResponseCodes.Success };
                        }
        ...
        public async Task<object> POST_OIL_CONDENSATE_PRODUCTION_ACTIVITY([FromBody] OIL_CONDENSATE_PRODUCTION_ACTIVITy oil_condensate_activity_model, string omlName, string fieldName, string year, string actionToDo, int id)
        {
            int save = 0;
            int Id = id == 0 ? oil_condensate_activity_model.Id : id;
            string action = (actionToDo == null || actionToDo == "") ? GeneralModel.Insert : actionToDo.Trim().ToLower();
            var concessionField = GET_CONCESSION_FIELD(omlName, fieldName);
            try
            {
        ...
        public async Task<object> POST_NIGERIA_CONTENT_Training([FromBody] NIGERIA_CONTENT_Training nigeria_content_training_model, string year, string actionToDo)
        {
            ...
            int save = 0;
            int Id = (nigeria_content_training_model.Id != null && nigeria_content_training_model.Id > 0) ? nigeria_content_training_model.Id : 0;
            string action = (actionToDo == null || actionToDo == "") ? GeneralModel.Insert : actionToDo.Trim().ToLower();
            //var concessionField = GET_CONCESSION_FIELD(omlName, fieldName);

            try
            {
                NIGERIA_CONTENT_Training getData;
                #region Saving NIGERIA_CONTENT_Trainings data
                if (nigeria_content_training_model != null)
                {
                    getData = await (from c in _context.NIGERIA_CONTENT_Trainings where c.COMPANY_ID == WKPCompanyId && c.Year_of_WP == year && c.Actual_Proposed == nigeria_content_training_model.Actual_Proposed && c.Actual_Proposed_Year == nigeria_content_training_model.Actual_Proposed_Year select c).FirstOrDefaultAsync();
                    ...
                    else if (Id > 0 && action == GeneralModel.Update)
                    {
                        getData = await (from c in _context.NIGERIA_CONTENT_Trainings where c.Id == Id select c).FirstOrDefaultAsync();
                        if (getData != null)
                        {
                            getData.Actual_Proposed = nigeria_content_training_model.Actual_Proposed;
                            getData.Actual_Proposed_Year = nigeria_content_training_model.Actual_Proposed_Year;
                            getData.Expatriate_quota_positions = nigeria_content_training_model.Expatriate_quota_positions;
                            getData.Management_Foriegn = nigeria_content_training_model.Management_Foriegn;
                            getData.Management_Local = nigeria_content_training_model.Management_Local;
                            getData.Nigerian_Understudies = nigeria_content_training_model.Nigerian_Understudies;
                            getData.Staff_Foriegn = nigeria_content_training_model.Staff_Foriegn;
                            getData.Staff_Local = nigeria_content_training_model.Staff_Local;
        ...
        public async Task<object> POST_HSE_ENVIRONMENTAL_STUDIES_NEW([FromBody] HSE_ENVIRONMENTAL_STUDIES_NEW hse_environmental_model, string omlName, string fieldName, string year, string id, string actionToDo)
        {
            ...
            int save = 0;
            int Id = !string.IsNullOrEmpty(id) ? int.Parse(id) : hse_environmental_model.Id;
            string action = (actionToDo == null || actionToDo == "") ? GeneralModel.Insert : actionToDo.Trim().ToLower();
            var concessionField = GET_CONCESSION_FIELD(omlName, fieldName);
                ...
                if (Id > 0)
                {
                    ...
                    if (getData != null)
                    {
                        ...
                        else
                        {
                            ...
                            getData.Any_Environmental_Studies = hse_environmental_model.Any_Environmental_Studies;
                            getData.Consession_Type = hse_environmental_model.Consession_Type;
                            getData.Contract_Type = hse_environmental_model.Contract_Type;
                            getData.If_Ongoing = hse_environmental_model.If_Ongoing;
                            getData.If_YES_state_Project_Name = hse_environmental_model.If_YES_state_Project_Name;
                            getData.NUPRC_Approval_Status = hse_environmental_model.NUPRC_Approval_Status;
                            getData.OML_ID = hse_environmental_model.OML_ID;
                            getData.Status_ = hse_environmental_model.Status_;
                            getData.Terrain = hse_environmental_model.Terrain;

                            _context.HSE_ENVIRONMENTAL_STUDIES_NEWs.Update(getData);
                            save += await _context.SaveChangesAsync();
        ...
        public async Task<object> POST_HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_QUESTION([FromForm] HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_QUESTION hse_sustainable_model, string omlName, string fieldName, string year, int id, string actionToDo)
        {
            ...
            try
            {
                ...
                else if (hse_sustainable_model != null)
                {
                    ...
                    {
                        hse_sustainable_model.MOUResponderFilePath = null;
                        hse_sustainable_model.MOUResponderFilename = null;
                    }
                    ...
                    {
                        hse_sustainable_model.MOUOSCPFilePath = null;
                        hse_sustainable_model.MOUOSCPFilename = null;
                    }
                    #endregion
                    ...
                    if (getData != null)
                    {
                        ...
                        else
                        {
                            if (hse_sustainable_model != null)
                            {
                                ...
                                getData.Component_of_project = hse_sustainable_model.Component_of_project;
                                getData.Actual_Budget_Total_Dollars = hse_sustainable_model.Actual_Budget_Total_Dollars;
                                getData.Beneficiary_Community = hse_sustainable_model.Beneficiary_Community;
                                getData.Consession_Type = hse_sustainable_model.Consession_Type;
                                getData.Contract_Type = hse_sustainable_model.Contract_Type;
                                getData.Field_ID = hse_sustainable_model.Field_ID;
                                getData.OML_ID = hse_sustainable_model.OML_ID;
                                getData.OML_Name = hse_sustainable_model.OML_Name;
                                getData.Project_Location = hse_sustainable_model.Project_Location;
                                getData.Status_Of_Project = hse_sustainable_model.Status_Of_Project;
                                getData.Type_of_project_excuted = hse_sustainable_model.Type_of_project_excuted;
                                getData.Year_GMou_was_signed = hse_sustainable_model.Year_GMou_was_signed;
                                ...
                                {
                                    hse_sustainable_model.MOUUploadFilePath = null;
                                }
                                #endregion
                                _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_MOUs.Update(getData);
                                save += _context.SaveChanges();
                                string successMsg = Messager.ShowMessage(action);
                                return new WebApiResponse { ResponseCode = AppResponseCodes.Success, Message = successMsg, StatusCode = ResponseCodes.Success };
                            }
        ...
        public async Task<object> POST_HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_PLANNED_AND_ACTUAL([FromBody] HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_PLANNED_AND_ACTUAL hse_sustainable_model, string year, int id, string actionToDo)
        {
            ...
            try
            {
                ...
                else if (hse_sustainable_model != null)
                {
                    ...
                    getData = await (from c in _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_PLANNED_AND_ACTUALs where c.COMPANY_ID == WKPCompanyId && c.Id == hse_sustainable_model.Id && c.Year_of_WP == year select c).FirstOrDefaultAsync();
        ...
        public async Task<object> POST_HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_TRAINING_SCHEME([FromForm] HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_TRAINING_SCHEME hse_sustainable_model, string year, int id, string actionToDo)
        {
            ...
            try
            {
                if (id > 0 && action == GeneralModel.Delete)
                {
                    var getData = (from c in _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_TRAINING_SCHEMEs where c.Id == id select c).FirstOrDefault();
        ...
        public async Task<object> POST_HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_CSR_NEW_Scholarship([FromBody] HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_CSR_NEW_Scholarship hse_sustainable_model, string year, int id, string actionToDo)
        {
            int save = 0;
            int Id = hse_sustainable_model.Id;
            string action = (actionToDo == null || actionToDo == "") ? GeneralModel.Insert : actionToDo.Trim().ToLower();
            ...
            try
            {
                if (id > 0 && action == GeneralModel.Delete)
                {
                    var getData = (from c in _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_CSR_NEW_Scholarships where c.Id == id select c).FirstOrDefault();
            ...
            try
            {
                ...
                if (hse_point_source_registration != null)
                {
                    ...
                    if (action == GeneralModel.Insert)
                    {
                        ...
                        if (id > 0)
                        {
                            action = GeneralModel.Update;
                            getData = (from c in _context.HSE_POINT_SOURCE_REGISTRATIONs where c.Id == id select c).FirstOrDefault();
                            _context.HSE_POINT_SOURCE_REGISTRATIONs.Remove(getData);

                            if (hse_point_source_registration.are_there_point_source_permit == "NO")
                            {
                                hse_point_source_registration.evidence_of_PSP_path = null;
                                hse_point_source_registration.evidence_of_PSP_filename = null;
                            }
        ...
        public async Task<object> POST_HSE_WASTE_MANAGEMENT_DZ([FromForm] HSE_WASTE_MANAGEMENT_DZ hSE_WASTE_MANAGEMENT_DZ, string omlName, string fieldName, string year, int id, string actionToDo)
        {
            ...
            try
            {
                ...
                if (id != null && id > 0)
                {
                    getData = (from c in _context.HSE_WASTE_MANAGEMENT_DZs where c.Id == id select c).FirstOrDefault();
                    ...
                    {
                        string successMsg = Messager.ShowMessage(action);
                        var All_Data = await (from c in _context.HSE_WASTE_MANAGEMENT_DZs where c.Id == id select c).ToListAsync();
                        return new WebApiResponse { ResponseCode = AppResponseCodes.Success, Message = successMsg, Data = All_Data, StatusCode = ResponseCodes.Success };
                    }
                }
                ...
                if (hSE_WASTE_MANAGEMENT_DZ != null)
                {
                    ...
                    hSE_WASTE_MANAGEMENT_DZ.OML_ID = concessionField.Concession_ID.ToString();
                    ...
                    else if (action == GeneralModel.Update && Id > 0)
                    {
                        getData = (from c in _context.HSE_WASTE_MANAGEMENT_DZs where c.Id == Id select c).FirstOrDefault();
                    ...
                    {
                        //return BadRequest(new { message = "Error : An error occured while trying to submit this form." });
                        return new WebApiResponse { ResponseCode = AppResponseCodes.InternalError, Message = "Error : An error occured while trying to submit this form", Data = hSE_WASTE_MANAGEMENT_DZ, StatusCode = ResponseCodes.InternalError };

                    }
                }

                // return BadRequest(new { message = $"Error : No data was passed for {actionToDo} process to be completed." });
                return new WebApiResponse { ResponseCode = AppResponseCodes.Failed, Message = $"Error : No data was passed for {actionToDo} process to be completed.", StatusCode = ResponseCodes.Badrequest };
        ...
        public async Task<object> POST_HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_SCHOLASHIP_SCHEME([FromForm] HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_SCHOLASHIP_SCHEME hse_scholarship_model, string year, int id, string actionToDo)
        {
            ...
            try
            {
                ...
                {
                    //int.Parse(id)
                    var getData = (from c in _context.HSE_SUSTAINABLE_DEVELOPMENT_COMMUNITY_PROJECT_PROGRAM_SCHOLASHIP_SCHEMEs where c.Id == id select c).FirstOrDefault();
