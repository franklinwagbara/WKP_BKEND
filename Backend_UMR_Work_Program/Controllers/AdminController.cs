...
namespace Backend_UMR_Work_Program.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<WebApiResponse> CreateUserNew([FromBody] ADMIN_COMPANY_INFORMATION_Model userModel)
        {
            ...
            try
            {
                var checkUser = await (from c in _context.ADMIN_COMPANY_INFORMATIONs
                                       where c.EMAIL.ToLower() == userModel.EMAIL.ToLower() && c.COMPANY_NAME == "Admin"
        ...
        [HttpPost("ADD_ADMIN_MEETING_ROOMS")]
        public async Task<WebApiResponse> Add_Admin_Meeting_Rooms([FromBody] ADMIN_MEETING_ROOM model)
        {
            try
            {
                int save = 0;
                ...
                if (save > 0)
                {
                    ...
                    return new WebApiResponse { ResponseCode = AppResponseCodes.Success, Message = $"{model.MEETING_ROOM} has been inserted successfully", Data = returnModel, StatusCode = ResponseCodes.Success };
                }
                else
                {
                    return new WebApiResponse { ResponseCode = AppResponseCodes.Failed, Message = $"Error: An error occured while trying to insert {model.MEETING_ROOM} meeting room.", StatusCode = ResponseCodes.Failure };
                }
        ...
        [HttpPost("DELETE_ADMIN_MEETING_ROOMS")]
        public async Task<WebApiResponse> Delete_Admin_Meeting_Rooms(int Id)
        {
            try
            {
                int save = 0;
                if (Id != null && Id > 0)
                {
                    var checkdata = _context.ADMIN_MEETING_ROOMs.Where(x => x.Id == Id).FirstOrDefault();
                    if (checkdata == null)
                    {
                        return new WebApiResponse { ResponseCode = AppResponseCodes.Failed, Message = $"Error: details could not be found on the portal. ", StatusCode = ResponseCodes.Failure };
                    }
                    else
                    {
                ...
                {
                    var returnModel = _context.ADMIN_MEETING_ROOMs.ToList();
                    return new WebApiResponse { ResponseCode = AppResponseCodes.Success, Message = $"data has been deleted successfully", Data = returnModel, StatusCode = ResponseCodes.Success };
                }
                else
                {
                    return new WebApiResponse { ResponseCode = AppResponseCodes.Failed, Message = $"Error: An error occured while trying to delete meeting room.", StatusCode = ResponseCodes.Failure };
                }
        ...
        public async Task<WebApiResponse> Admin_Meeting_Rooms([FromBody] ADMIN_MEETING_ROOM model, string action)
        {
            try
            {
                int save = 0;
